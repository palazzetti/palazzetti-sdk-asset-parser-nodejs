...
import * as asset_parser from "../assets/asset_parser.json"
...
export class AssetParser {
    ...
    constructor({get_alls, get_stdt} : {
        get_alls?: any,
        get_stdt: any
    }) {
        this._grammar = asset_parser;
        this._alls = get_alls;
        this._stdt = get_stdt;
    }
    ...
    private value(dictName: AssetParserDictionary, key: string) {
        let k = key;
    	let sourceDict = this.dict(dictName);
    ...
    private evaluate({
        ...
        let _optionalValue = this.value(statementPath, statementKey);
    	let _operator = statementOperator ?? "".toLowerCase();
    	...
    	switch (_operator) {
    		...
    		case "vgt":
    			try {
    				return (semverCompare(this.formattedVersion(_optionalValue), this.formattedVersion(statementValue)) > 0);
    			} catch (ex) { return false; };
    			break;
    		case "vgte":
    			try {
    ...
    private parse(): Capabilities {
    	...
    	Object.keys(this._grammar).forEach((currKey) => {
    		...
    		if (typeof currValue === 'object') {
    			...
    			let _rawValue = this.value(currValue["path"], currValue["key"]);
    			...
    			let _sourceArray = currValue["map_keys"];
    			let _index = parseInt(_rawValue);
    		...
    		// Is a numeric value
    		if (!isNaN(currValue)) {
    			_parsedObject[`${currKey}`] = (Number.isInteger(currValue) ? parseInt(currValue) : parseFloat(currValue).toFixed(2));
    			return;
    		}
